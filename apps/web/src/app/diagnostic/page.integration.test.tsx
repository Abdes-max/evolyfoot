...
describe("initial diagnostic",()=>{it("explique les deux priorités au coach",()=>{render(<DiagnosticPage/>);fireEvent.click(screen.getByRole("button",{name:"Réagir après la perte : Rarement"}));fireEvent.click(screen.getByRole("button",{name:/voir mes priorités/i}));expect(screen.getByRole("status")).toHaveTextContent("Réagir après la perte");expect(screen.getByRole("status")).toHaveTextContent("Récupérer rapidement");});});
