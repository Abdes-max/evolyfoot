...
import { createChangeFormationHandler, createMatchGateway } from "@/server/match";
...
export async function PUT(request: Request, { params }: RouteParams): Promise<Response> {
  ...
  try {
    return await createChangeFormationHandler(resolveEducator, gateway, console.error)(request, id);
  } finally {
    await disconnect();
  }
}
