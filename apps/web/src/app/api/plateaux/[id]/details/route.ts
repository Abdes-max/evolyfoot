...
import { createPlateauGateway, createUpdatePlateauDetailsHandler } from "@/server/plateau";
...
export async function PATCH(request: Request, { params }: RouteParams): Promise<Response> {
  ...
  try {
    return await createUpdatePlateauDetailsHandler(resolveEducator, gateway, console.error)(request, id);
  } finally {
    await disconnect();
  }
}
