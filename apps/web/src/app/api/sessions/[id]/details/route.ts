...
import { createTrainingSessionGateway, createUpdateTrainingSessionDetailsHandler } from "@/server/training-session";
...
export async function PATCH(request: Request, { params }: RouteParams): Promise<Response> {
  ...
  try {
    return await createUpdateTrainingSessionDetailsHandler(resolveEducator, gateway, console.error)(request, id);
  } finally {
    await disconnect();
  }
}
