...
import { createGetTournamentHandler, createRemoveTournamentHandler, createTournamentGateway } from "@/server/tournament";
...
export async function GET(request: Request, { params }: RouteParams): Promise<Response> {
  ...
  try {
    return await createGetTournamentHandler(resolveEducator, gateway, console.error)(request, id);
  } finally {
    await disconnect();
  }
}
