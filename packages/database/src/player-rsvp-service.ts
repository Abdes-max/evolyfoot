...
export class PlayerRsvpService {
  ...
  async respondToMatch(playerAccountId: string, matchId: string, status: AttendanceStatus): Promise<void> {
    ...
    attendance.push(createAttendanceEntry(playerId, owned.name, status));
