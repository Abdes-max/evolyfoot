...
`/admin/metrics` opérationnalise deux des quatre indicateurs de `docs/mvp.md` — l'entonnoir d'activation (comptes créés → équipe → diagnostic → première séance → première observation, par nombre d'éducateurs distincts) et l'utilisation hebdomadaire sur quatre semaines consécutives. Les deux autres (« séance planifiée en moins de 10 minutes », « au moins un ajustement accepté par cycle ») restent volontairement absents : ils demanderaient une instrumentation qui n'existe pas encore (chronométrage de l'interaction, persistance de la décision de `AdjustmentCard`, aujourd'hui un état purement local jamais envoyé au serveur) plutôt que d'être approximés par une donnée qui mesure autre chose.

`MetricsService` (`packages/database/src/metrics-service.ts`) prend `PrismaClient` directement plutôt qu'un repository comme les autres services : il n'y a ici aucune règle métier à isoler derrière une interface pour un faux en mémoire, seulement de la lecture agrégée sur les tables existantes (`educators`, `teams`, `diagnostics`, `training_sessions`, `observations`, `players`) — testée contre un vrai Postgres plutôt qu'unitairement. Le regroupement par semaine calendaire (`date_trunc`) n'a pas d'équivalent dans le constructeur de requêtes Prisma : SQL brut pour ces deux requêtes précisément, sans valeur interpolée (aucun risque d'injection), avec des `::int` explicites partout — Postgres retourne les agrégats en `bigint`, que `JSON.stringify` ne sait pas sérialiser.

`GET /api/admin/metrics` exige une session **et** que l'e-mail de l'éducateur figure dans `ADMIN_EMAILS` (liste séparée par des virgules, vide par défaut — refuse tout le monde plutôt que d'exposer des données agrégées sans configuration explicite). 403 plutôt que 404 : ce n'est pas un secret que la route existe, seulement que son contenu l'est.
