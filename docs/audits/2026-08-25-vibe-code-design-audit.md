...
| 5 | Ombres portées sur tout | TROUVÉ | L'ombre décorative de `.focus-card` et l'anneau en `box-shadow` de `.dot.filled` sont dans `apps/web/src/app/globals.css:21,24`. Elles ne sont pas généralisées, mais ne signalent aucune élévation. |
...
| 9 | Tirets cadratins dans les textes | TROUVÉ | Tiret demi-cadratin dans `apps/web/src/app/page.tsx:22` et cadratin dans `apps/web/src/app/layout.tsx:7`. |
| 10 | Inter, Geist ou Space Grotesk comme police unique | TROUVÉ | `apps/web/src/app/globals.css:4` déclare Manrope puis Inter dans une pile sans chargement explicite. Décision typographique requise. |
...
| 19 | Même `border-radius` sur tout | TROUVÉ | Les cartes principales montent à 23 px et plusieurs panneaux secondaires à 18 px dans `apps/web/src/app/globals.css:21,24`, malgré quelques rayons différenciés. |
...
- Supprimer les ombres décoratives.
- Retirer les glyphes décoratifs sans introduire de bibliothèque d'icônes.
- Remplacer les tirets cadratins concernés.
- Hiérarchiser les rayons avec un plafond de 16 px pour les grandes cartes.
...
|---|---:|---|
| Accessibilité | 3/4 | Les contrastes textuels mesurés atteignent désormais au moins 4,5:1 ; l'audit ne remplace pas un passage complet au clavier et lecteur d'écran. |
...
Contrastes remesurés : texte secondaire/papier 5,07:1 ; texte secondaire/surface 5,47:1 ; étapes onboarding/vert 4,52:1 ; score intermédiaire 7,40:1 ; score prioritaire 6,57:1 ; erreur/pastel 6,31:1.
